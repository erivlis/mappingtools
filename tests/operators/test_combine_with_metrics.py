from mappingtools.operators import combine_with_metrics
...
def test_combine_with_provenance_single_metric():
    ...
    combined, metrics = combine_with_metrics(t1, t2, Resolver.LAST, [DecisionMetric.PROVENANCE])
...
def test_combine_with_provenance_first_wins():
    ...
    combined, metrics = combine_with_metrics(t1, t2, Resolver.FIRST, [DecisionMetric.PROVENANCE])
...
def test_combine_with_audit_metric():
    ...
    combined, metrics = combine_with_metrics(t1, t2, Resolver.LAST, [DecisionMetric.AUDIT])
...
def test_combine_with_changelog_metric():
    ...
    combined, metrics = combine_with_metrics(t1, t2, Resolver.LAST, [DecisionMetric.CHANGELOG])
...
def test_combine_with_metrics_void_resolver():
    t1 = {"a": 1, "b": 2}
    t2 = {"a": 99}

    combined, metrics = combine_with_metrics(t1, t2, Resolver.VOID, [DecisionMetric.PROVENANCE])
    assert combined == {"b": 2}
    assert metrics["PROVENANCE"] == {"b": 0}


def test_combine_with_metrics_null_resolver():
    t1 = {"a": 1}
    t2 = {"a": 99}

    combined, metrics = combine_with_metrics(t1, t2, Resolver.NULL, [DecisionMetric.PROVENANCE])
    assert combined == {"a": None}
    assert metrics["PROVENANCE"] == {"a": None}


def test_combine_with_metrics_aggregative_resolver():
    t1 = {"a": 10}
    t2 = {"a": 20}

    combined, metrics = combine_with_metrics(t1, t2, NumericResolver.SUM, [DecisionMetric.PROVENANCE])
    assert combined == {"a": 30}
    assert metrics["PROVENANCE"] == {"a": None}


def test_combine_with_metrics_mismatched_structures():
...
def test_combine_with_metrics_none_metrics():
    t1 = {"a": 1}
    t2 = {"a": 2}
    combined, metrics = combine_with_metrics(t1, t2, Resolver.LAST, None)
    assert combined == {"a": 2}
    assert metrics == {}


def test_combine_with_metrics_custom_callable_op():
    t1 = {"a": 10}
    t2 = {"a": 20}
    combined, metrics = combine_with_metrics(
...
def test_combine_with_metrics_resolved_container_t2():
    t1 = {"a": 10}
    t2 = {"a": {"b": 20}}
    combined, metrics = combine_with_metrics(t1, t2, Resolver.LAST, [DecisionMetric.PROVENANCE])
...
def test_combine_with_metrics_resolved_container_new():
    t1 = {"a": 10}
    t2 = {"a": 20}
    ...
    combined, metrics = combine_with_metrics(t1, t2, custom_resolver, [DecisionMetric.PROVENANCE])
...
def test_changelog_metric_unchanged_on_conflict():
    t1 = {"a": 1}
    t2 = {"a": 2}
    combined, metrics = combine_with_metrics(t1, t2, Resolver.FIRST, [DecisionMetric.CHANGELOG])
    assert combined == {"a": 1}
...
def test_decision_metric_of_method():
    ...
    t1 = {"a": 1, "b": 2, "c": [10, 20]}
    t2 = {"a": 99, "b": 2, "c": [10]}
    combined, _ = combine_with_metrics(t1, t2, Resolver.VOID, [])
