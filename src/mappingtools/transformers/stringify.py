...
def _stringify_mapping(obj: Mapping, *args, **kwargs) -> str:
    return _stringify_kv_stream(obj.items(), *args, **kwargs)


def _stringify_iterable(obj: Iterable, processor, kv_delimiter, item_delimiter, *args, **kwargs) -> str:
    return f'[{item_delimiter.join(processor(v) for v in obj)}]'


def _stringify_class(obj, processor, kv_delimiter, item_delimiter, *args, **kwargs) -> str:
